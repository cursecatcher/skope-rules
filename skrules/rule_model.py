...
#        print(f"Positive rules kept: {len(selected_pos)} negative rules kept: {}")
    ...
    def test_rules(self, X: pd.DataFrame, y):
        tester = RuleTester(self.ruleset)
        return tester.exec_rules(X, y)
...
class RuleTester:
    ...
    def exec_rules(self, X: pd.DataFrame, y):
        ...
        for main_rule, rules in rule_collection.items():
            ...
            for other in rules:
                # print("Clause")
                evaluation.append(other.eval(X, pos, neg))
            
            # print("\n")
            
            # if len(rules) > 2:
            #     rules[0].intersect(rules[1])
